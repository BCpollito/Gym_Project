    ...
    <>
      <div className="fixed top-0 left-0 w-full px-4 pt-2 bg-white shadow z-50">
        <div className="flex items-center justify-between w-full">
          ...
          <IconButton onClick={toggleOpen} className='mb-2' variant="text" size="lg">
           {open ? <X className="h-6 w-6"/> : <ListIcon className="h-6 w-6"/>}
          </IconButton>
        </div>
        <Collapse open={open}>
          <List>
            <ListItem onClick={handleLinkWorkout} className="justify-center text-center text-gray-500">
              <ListItemPrefix>
                <Dumbbell className="h-4 w-4" />
              </ListItemPrefix>
              <Typography className="text-sm">Actividades</Typography>
            </ListItem>
            <ListItem onClick={handleLinkPograms} className="justify-center text-center text-gray-500">
              <ListItemPrefix>
